...
consistency_score = st.sidebar.slider('Overall Consistency Score', min_value=int(df['Result.OverallConsistencyScore'].min()), max_value=int(df['Result.OverallConsistencyScore'].max()), value=(int(df['Result.OverallConsistencyScore'].min()), int(df['Result.OverallConsistencyScore'].max())))
...
filtered_df = df[
    ...
    (df['Result.OverallConsistencyScore'] >= consistency_score[0]) & (df['Result.OverallConsistencyScore'] <= consistency_score[1])
]
...
for idx, row in filtered_df.iloc[start_idx:end_idx].iterrows():
    ...
    st.write(f"**Overall Consistency Score:** {row['Result.OverallConsistencyScore']}")
    st.write("---")
